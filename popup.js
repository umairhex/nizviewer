  ...
  document.addEventListener("DOMContentLoaded", () => {
        ...
        if (browserApi.tabs) {
          browserApi.tabs.query({ url: ["*://*.indeed.com/*", "*://*.indeed.*/*"] }, (tabs) => {
            if (tabs) tabs.forEach(t => {
              try { browserApi.tabs.sendMessage(t.id, { type: "PREFS_CHANGED", prefs }); } catch (e) {}
            });
          });
        }
      } catch (e) {
    ...
    if (btnSelectAll) {
      btnSelectAll.addEventListener("click", async () => {
        ...
        await saveAndBroadcast();
        const orig = btnSelectAll.textContent;
        btnSelectAll.textContent = "✓";
        setTimeout(() => { btnSelectAll.textContent = orig; }, 800);
      });
    }
    ...
    if (btnSelectNone) {
      btnSelectNone.addEventListener("click", async () => {
        ...
        await saveAndBroadcast();
        const orig = btnSelectNone.textContent;
        btnSelectNone.textContent = "✓";
        setTimeout(() => { btnSelectNone.textContent = orig; }, 800);
      });
    }
      ...
      btnClearCache.addEventListener("click", async () => {
        ...
        try {
          ...
          if (browserApi.tabs) {
            browserApi.tabs.query({ url: ["*://*.indeed.com/*", "*://*.indeed.*/*"] }, (tabs) => {
              if (tabs) tabs.forEach(t => {
                try { browserApi.tabs.sendMessage(t.id, { type: "CACHE_CLEARED" }); } catch (e) {}
              });
            });
          }
        } catch (e) {
    ...
    toggle.addEventListener("change", async () => {
      ...
      try {
        ...
        if (browserApi.tabs) {
          browserApi.tabs.query({ url: ["*://*.indeed.com/*", "*://*.indeed.*/*"] }, (tabs) => {
            if (tabs) tabs.forEach(t => {
              try { browserApi.tabs.sendMessage(t.id, { type: "EXTENSION_STATE_CHANGED", enabled: isEnabled }); } catch (e) {}
            });
          });
        }
      } catch (e) {
